...
sign_in_btn.onclick = () => {
  ...
  page.innerHTML = "1/4";
  alert(
    "You have to attempt all questions and if you don't then zero mark will be considered for that question."
  );
};
...
fifth_page_btn.onclick = () => {
  ...
  if (correct_point >= 7) {
    alert(
      "you have passed the test,click the next button below to view your result"
    );
    status.innerHTML = "Pass";
  } else {
    alert(
      "you have failed the test,click the next button below to view your result"
    );
...
function right_selection(value) {
  ...
  // Define the necessary variables (username, userid, correctQue, wrongQue, attQue, totalQue)
  const username = document.forms["form-1"]["username"].value;
  const userid = document.forms["form-1"]["userid"].value;
  const correctQue = correct_point;
  const wrongQue = 20 - correct_point;
  const attQue = correct_point + (20 - correct_point);
  const totalQue = 20;
  ...
  document.querySelector(".correct_que").textContent = correctQue;
  document.querySelector(".wrong_que").textContent = wrongQue;
  document.querySelector(".att_que").textContent = attQue;
  document.querySelector(".status").textContent =
    correctQue >= totalQue / 2 ? "Pass" : "Fail";

  shareLinks.forEach((link) => {
    link.addEventListener("click", (event) => {
      event.preventDefault();
      const url = window.location.href;
      const socialMedia = link.querySelector("i").classList[1];
      const shareUrl = generateShareUrl(socialMedia, url);
      window.open(shareUrl, "_blank");
    });
  });

  function generateShareUrl(socialMedia, url) {
    switch (socialMedia) {
      case "fa-facebook-f":
